...
class MenuControllerStyle13: UIViewController, UITableViewDelegate, UITableViewDataSource, UICollectionViewDelegate, UICollectionViewDataSource, MenuStyle1HeaderDelegate {
    ...
    func createView(){
        ...
        // Create MenuShow
        menuShow = (storyboard?.instantiateViewController(withIdentifier: "MenuLeftControllerStyle13"))!
        addChildViewController(menuShow)
        menuShow.view.frame = CGRect(x: -self.view.frame.size.width, y: 80, width: self.view.frame.size.width/1.2, height: self.view.frame.size.height-80)
    ...
    func menu() { // function for menu
        if show {
            ...
            UIView.animate(withDuration: 0.5, animations: {
                self.menuShow.view.frame.origin.x = -self.view.frame.size.width/1.2
