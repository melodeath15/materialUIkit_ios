...
class MenuControllerStyle1: UIViewController, UITableViewDelegate, UITableViewDataSource, UICollectionViewDelegate, UICollectionViewDataSource, MenuStyle1HeaderDelegate, MenuStyle1ViewMenuDelegate {
    ...
    var menuShow = MenuStyle1ViewMenu() // var view xib in folder menu style1 name MenuStyle1ViewMenu
    var show = true
    
    override func viewDidLoad() {
        super.viewDidLoad()
        
        createView() // function for create create table and regis UINib, create menu show, header
    ...
    func createView(){
        ...
        // Create Menu Show
        menuShow = Bundle.main.loadNibNamed("MenuStyle1ViewMenu", owner: nil, options: nil)?.first as! MenuStyle1ViewMenu
        menuShow.frame = CGRect(x: 0, y: -250, width: self.view.frame.size.width, height: 250)
        menuShow.delegate = self
    ...
    func menu() { // function for menu
        if show {
            UIView.animate(withDuration: 0.5, animations: {
                self.menuShow.frame.origin.y = 80
            ...
            UIView.animate(withDuration: 0.5, animations: {
                self.menuShow.frame.origin.y = -250
    ...
    func message() {
        FunctionDefault.AlertMessageAction(title: "Message", message: "", targetVC: self)
    }
    ...
    func notif() {
        FunctionDefault.AlertMessageAction(title: "Notification", message: "", targetVC: self)
    }
    // end
