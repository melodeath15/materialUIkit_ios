...
class MenuControllerStyle18: UIViewController, UITableViewDelegate, UITableViewDataSource, UICollectionViewDelegate, UICollectionViewDataSource, MenuStyle1HeaderDelegate {
    ...
    func createView(){
        ...
        // Create MenuShow
        menuShow = (storyboard?.instantiateViewController(withIdentifier: "MenuLeftControllerStyle18"))!
        addChildViewController(menuShow)
        menuShow.view.frame = CGRect(x: -self.view.frame.size.width, y: 80, width: self.view.frame.size.width, height: self.view.frame.size.height/1.5)
