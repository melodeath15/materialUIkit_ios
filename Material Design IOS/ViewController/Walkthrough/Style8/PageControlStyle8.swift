...
class PageControlStyle8: UIViewController, WalkthroughStyle8FooterDelegate {
    ...
    // create var delegate pageViewControllerStyle8
    var pageViewControllerStyle8: PageViewControllerStyle8? {
        didSet {
            pageViewControllerStyle8?.pageStyle8Delegate = self
        }
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let PageViewController = segue.destination as? PageViewControllerStyle8 {
            self.pageViewControllerStyle8 = PageViewController
        }
    }
    
    // func change pagecontroll
    func didChangePageControlValue() {
        pageViewControllerStyle8?.scrollToViewController(index: pageControll.currentPage)
    }
...
// func PageControlStyle8Delegate
extension PageControlStyle8: PageViewControllerStyle8Delegate {
    // func
