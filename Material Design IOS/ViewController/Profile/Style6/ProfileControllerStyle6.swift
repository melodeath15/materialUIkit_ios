...
class ProfileControllerStyle6: UIViewController, UITableViewDelegate, UITableViewDataSource, ProfileHeaderDelegate, ProfileHeaderStyle6Delegate, ProfileTapCollectionDelegate, ProfileCellStyle6Delegate {
    
    var headerMenu = ProfileHeader()
    var header = ProfileHeaderStyle6()
    ...
    func createView(){
        ...
        // Create Header
        header = Bundle.main.loadNibNamed("ProfileHeaderStyle6", owner: nil, options: nil)?.first as! ProfileHeaderStyle6
        header.frame = CGRect(x: 0, y: 80, width: self.view.frame.size.width, height: 100)
        ...
        tap.frame = CGRect(x: 0, y: 180, width: self.view.frame.size.width, height: 60)
        tap.counterMenu = 3
        tap.active = 0
        status = 0
