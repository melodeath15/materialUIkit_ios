//
//  ProfileControllerStyle18.swift
//  Material Design IOS
//
...
class ProfileControllerStyle18: UIViewController, UICollectionViewDelegateFlowLayout, UICollectionViewDataSource, ProfileHeaderDelegate, CHTCollectionViewDelegateWaterfallLayout, GalleryStyle3CellDelegate {
    
    var collMenu: UICollectionView!
    var header = ProfileHeader()
    var headerCell = ProfileHeaderCellStyle18()
    ...
    func createView(){
        ...
        // Create headerCell
        headerCell = Bundle.main.loadNibNamed("ProfileHeaderCellStyle18", owner: nil, options: nil)?.first as! ProfileHeaderCellStyle18
        headerCell.frame = CGRect(x: 0, y: 80, width: self.view.frame.size.width, height: 180)
        headerCell.bg.downloadedFrom(link: "\(baseAssetProfile)Profile-18-header.jpg", contentMode: .scaleAspectFill)
        ...
        collMenu = UICollectionView(frame: CGRect(x: 0, y: 260, width: self.view.frame.size.width, height: self.view.frame.size.height-260), collectionViewLayout: layout)
